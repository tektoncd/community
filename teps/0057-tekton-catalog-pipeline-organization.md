...
# TEP-0057: Tekton Catalog Pipeline Organization
...
- If there's a new version of the task then it completely depends on the author
  of the pipeline whether to bump the version of the pipeline or not
...
# The install script which will install all the related resources
./{resource-type}/{resource-name}/{version}/install-script.sh

# Tasks directory which refers to the task used in the pipeline
./{resource-type}/{resource-name}/{version}/tasks/..
...
- Pipeline can have tasks which are part of tekton catalog and they can be referred via annotations

- Pipeline may have tasks out of which some tasks might come from the catalog and some
  may be specific to that pipeline which have to be added to the `/tasks` dir

- Pipeline can have tasks which are just specific to that pipeline and that
  tasks can be added in the `/tasks` directory
