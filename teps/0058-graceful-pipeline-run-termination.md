...
users update its definition with states like:

- "PipelineRunCancel" - cancel `PipelineRun` and ensure `finally` is run
- "PipelineRunStop" - stop `PipelineRun` and ensure `finally` is run
...
------ | ---------------------------- | -----------------------------
cancel | PipelineRunCancelSkipFinally | PipelineRunCancel
stop   |                              | PipelineRunStop

The existing state "PipelineRunCancelled" is deprecated and replaced by "PipelineRunCancelSkipFinally".

We need to consider following cases:

1. User sets `spec.status` to "PipelineRunCancel" in `PipelineRun` with running tasks, 
...
2. User sets `spec.status` to "PipelineRunCancel" in `PipelineRun` with running tasks 
...
3. User sets `spec.status` to "PipelineRunStop" in `PipelineRun` with running tasks.
    ...
    - when final tasks are completed, `PipelineRun` condition is:
      ```yaml
      { type: Succeeded, Status: False, Reason: PipelineRunStopped } 
      ```

4. User sets `spec.status` to "PipelineRunCancel" or "PipelineRunStop" 
...
5. User sets `spec.status` to "PipelineRunCancel" or "PipelineRunStop" 
...
- If `PipelineRun` has stopped executing (i.e. [the Succeeded Condition is False or True](https://github.com/tektoncd/pipeline/blob/main/docs/pipelineruns.md#monitoring-execution-status)),
  ...
  for "PipelineRunCancelled" state (replaced by "PipelineRunCancelSkipFinally").
  
- "PipelineRunCancelSkipFinally" - if this state is set when the `PipelineRun` is already in "PipelineRunStopping" state,
...
"PipelineRunCancel" and "PipelineRunStop" states changes the `finally` behaviour, 
which becomes an exit handler responsible for cleanup actions. 

In the future, somebody may be interested in support for "PipelineRunStopSkipFinally" state, 
