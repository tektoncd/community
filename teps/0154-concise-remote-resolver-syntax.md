...
- [Alternatives](#alternatives)
  - [Priority based Scheme Subscription](#priority-based-scheme-subscription)
<!-- /toc -->
...
This mapping of `scheme` to `resolver` could be easily maintained in a `configMap` for the cluster. This `configMap` is auto-updated by the resolvers upon [Initialization](https://tekton.dev/docs/pipelines/how-to-write-a-resolver/#the-initialize-method). This way, cluster operators do not need to maintain this `configMap`.
...
### Priority based Scheme Subscription

The cluster operators will be required to know the remote resolvers that are running on the cluster and the schemes they are subscribed to.  
The configMap must list out all the schemes supported by all the resolvers in [order of priority](#prioritizing-the-order-of-resolvers).
...
If the cluster operators do not provide this configMap, then the Tekton controller can only rely on the [built-in resolvers](#built-in-resolvers). This is because only the names of the built-in resolvers, not custom resolvers.  
